...
contract RchMgrInterface {
    ...
    constructor() public {
        owner = msg.sender;
        version = "RchMgrInterface version 1.0";
    ...
    function SetContract(address contractAddr,  string memory name ) public {
        require(IsAdmin(msg.sender) , "only admin can do it !");
        contractAddrs[name] = contractAddr ;
    }


    function GetContractAddr(string memory name ) public view returns (address ret) {
        return contractAddrs[name];
    }

}


contract RchRegPoolMgr {
    ...
    event RchTransfer(address indexed _from, address indexed _to, uint indexed value);

    uint256 constant Limit10000 = (10000*10**uint256(18));
    //uint256 constant ETHV_REDUCE_BOUNDING = 1110858 ;  // 半年减半机制
    uint256 public   unstake_time = 3600*24*30 ; // 3600*24*30

    constructor() public {
        owner = msg.sender;
        version = "RchRegPoolMgr version 1.5";
        adminList[owner] = true ;
    ...
    function SetStakeTime(uint256 unstakeTime) public {
        require(IsAdmin(msg.sender) , "only admin can do it !");
        require(unstakeTime >= 600 , "at least 600 secondes ");
        unstake_time = unstakeTime ;

    }


    function GetStakeGateValue() public pure returns (uint256 ret)  {
        //ETHV_REDUCE_BOUNDING = 1110858  // 半年减半机制
        //uint256 redu = SafeMath.div(block.number, ETHV_REDUCE_BOUNDING);
        ret = Limit10000 ;// SafeMath.mul(Limit10000, SafeMath.pwr(2, redu));
        return ret;
    }
    ...
    function RchUnStakePoolValue()external payable  {
        ...
        if( poolStakeList[msg.sender].userBalane > 0 ){
            ...
            emit RchTransfer( address(this) , msg.sender, poolStakeList[msg.sender].userBalane);
